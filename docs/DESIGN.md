...
- `background.js` が `chrome.webNavigation.onBeforeNavigate` で観測した「リダイレクト前の
  元URL」に `paipu=` が含まれることを確認できたページでのみ、記録が開始される
  (対局中の使用を拡張機能側で禁止する)。判定が後から覆った場合は、それまでのデータを
  即座に破棄する。
- 記録の開始自体はページ読み込み時に自動的に行う(理由: 雀魂の起動処理中に牌譜取得の
  通信が完了してしまうため、ユーザーがボタンを押すのを待つ設計では実用上間に合わないことを
  実機検証で確認した)。ユーザーの手動操作(開始/停止/クリア/エクスポートボタン)は、
  自動判定に失敗した場合のオーバーライドと、記録データを実際にJSONへ書き出す操作
  (これは常にユーザーが明示的に押した場合にのみ行われる)に用いる。

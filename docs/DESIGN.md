...
[`browser-extension/`](../browser-extension/) に、雀魂ブラウザ版の**牌譜(リプレイ)画面**で
WebSocket通信をキャプチャしJSON化できるChrome拡張を用意している。設計上の要点:

- 記録はページ読み込みと同時に**無条件で暫定的に**開始する(理由: 雀魂の起動処理中に
  牌譜取得の通信が完了してしまい、ユーザーがボタンを押すのを待つ設計はもちろん、
  「牌譜画面と確認できてから記録開始」という設計でも、確認の問い合わせ(数ミリ秒)が
  返る前に肝心の通信が終わってしまうことを2度の実機検証で確認した)。
- `background.js` が `chrome.webNavigation.onBeforeNavigate` で観測した「リダイレクト前の
  元URL」に `paipu=` が含まれることを確認できた場合のみ、暫定記録を確定として保持する。
  確認の結果「牌譜画面ではなかった」と判明した場合は、それまでのデータを即座に破棄し
  記録も停止する(対局中の使用を拡張機能側で禁止する仕組み)。
  さらに、JSONへの書き出し操作自体も、牌譜画面と確認できた場合(またはユーザーが
  手動オーバーライドで明示的に確認した場合)以外は拒否するガードを設けており、
  対局中データが暫定記録の間に外部へ出ることはない。
- ユーザーの手動操作(開始/停止/クリア/エクスポートボタン)は、自動判定に失敗した場合の
  オーバーライドと、記録データを実際にJSONへ書き出す操作(これは常にユーザーが
  明示的に押した場合にのみ行われる)に用いる。
